...
@router.get("/", response_model=ApiResponse[List[CompanyInDB]])
def list_companies(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), repo: CompanyRepository = Depends(get_company_repo)):
